...
const ChatItem: React.FC<{item: Chat}> = ({item}) => {
    //임시 이름
    ...
    <View style={styles.container}>
        {item.type === Type.MESSAGE ? (
            <View>
                {(item.sender !== tempUser && item.isFirstMessage) || !item.isSameTimeMessage ? (
                ...
                <View style={[styles.messageWrapper, {justifyContent: item.sender === tempUser ? 'flex-end' : 'flex-start'}]}>
                    {(!item.isSameTimeMessage && item.sender === tempUser) ? (
                    ...
                    {((item.isLastMessage && !item.isSameTimeMessage) || item.isFirstMessage) && item.sender !== tempUser ? (
                        <BasicText style={styles.timeText} text={item.time.getHours() + ':' + item.time.getMinutes()}/>
                    ) : null}
                </View>
            </View>

...
import { OTTItemType, GaldaeApiResponse } from '../../../types/getTypes';
// redux
...
const OTTNDivide: React.FC<HomeProps> = () => {
    ...
    const categoryOptions = [
        { id: 'ott', text: 'OTT', onPress: () => handleCategoryPress('ott') },
        { id: 'music', text: '음악', onPress: () => handleCategoryPress('music') },
        { id: 'productivity', text: '생산성', onPress: () => handleCategoryPress('productivity') },
        { id: 'education', text: '교육', onPress: () => handleCategoryPress('education') },
        { id: 'etc', text: '기타', onPress: () => handleCategoryPress('etc') },
    ];
    ...
    const handleFilterPopupConfirm = (
        ...
        const filterOptionsObj = {
            ...
            selectedTimeDiscuss,
            passengerNumber,
            selectedCategory: filterOptions.selectedCategory, // 기존 카테고리 상태 유지
        };
    ...
    const handleLongPress = (post: OTTItemType) => {
    ...
    //const displayedPosts: OTTItemType[] = searchResults ? searchResults.content : reduxPosts;
    ...
    //     // 카테고리 필터링은 현재 데이터에 category 속성이 없으므로 주석 처리
    //     // if (filterOptions.selectedCategory) {
    //     //     filtered = filtered.filter(item => item.category === filterOptions.selectedCategory);
    //     // }
    ...
    const finalFilteredData: OTTItemType[] = [
        {
            postId: '1',
            postService: '넷플릭스',
            postType: 'OTT',
                    ...
                    <View style={styles.filters}>
                        ...
                        {categoryOptions.map((option) => (
                            <SelectTextButton
                                key={option.id}
                                text={option.text}
                                selected={filterOptions.selectedCategory === option.id}
                                unselectedColors={{
                                    backgroundColor: theme.colors.white,
                                    textColor: theme.colors.blackV3,
                                    borderColor: theme.colors.blackV3,
                ...
                {reduxLoading || isLoadingMore ? (
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <OTTItem
                                item={item}
                                onPress={!item.isSameGender && item.passengerGenderType === 'SAME' ? () => setSameGenderPopupVisible(true) : () => navigation.navigate('OTTDetail', { postId: item.postId })}

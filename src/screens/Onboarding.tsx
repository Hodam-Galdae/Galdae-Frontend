...
const OnBoarding1 = () => {
  ...
  const position1 = useRef(new Animated.ValueXY({ x: -153, y: height - 350})).current;
  const position2 = useRef(new Animated.ValueXY({ x: width + 185 , y: height + 80 })).current;
  ...
  return(
    <View style={styles.container}>
      ...
      <BasicText text="같이 갈 그대, 갈대" style={[styles.title, {color: theme.colors.brandColor}]}/>
      ...
      <Svg height="542" width="542">
        <Defs>
          <RadialGradient
            ...
            cx="50%" cy="50%" r="50%"
            fx="50%" fy="50%">
            <Stop offset="0%" stopColor="rgba(218, 255, 205, 0.60)" stopOpacity="1" />
            <Stop offset="100%" stopColor="rgba(255, 255, 255, 0.60)" stopOpacity="1" />
          </RadialGradient>
        </Defs>
...
const OnBoarding2 = () => {
  return(
    <View style={[styles.container, {backgroundColor: theme.colors.brandColor}]}>
      <SVG name="LogoYellow"/>
    </View>
  );
};
...
const OnBoarding3 = () => {
  ...
  return (
    <View style={styles.wrapper}>
      <Animated.View style={[styles.hand1, {top: height - 150, opacity, transform: [{translateX : position4}]}]}>
        <SVG name="Balloon"/>
      </Animated.View>
      <Animated.View style={[styles.hand1, {top: height - 50, opacity, transform: [{translateX : position5}]}]}>
        <SVG name="Balloon"/>
      </Animated.View>
      <Animated.View style={[styles.hand1, {left: width / 2 - 100, opacity, transform: [{ translateY: position1 }]}]}>
        <SVG name="LocationGreen"/>
      </Animated.View>
      <Animated.View style={[styles.hand1, {left: width / 2 + 70, opacity, transform: [{ translateY: position2 }]}]}>
        <SVG name="LocationGreen"/>
      </Animated.View>
      <Animated.View style={[styles.hand1, {left: width / 2 - 138, opacity, transform: [{ translateY: position3 }]}]}>
        <SVG name="CarAni"/>
      </Animated.View>

      <View style={styles.container}>
      <BasicText text={'언제 어디서든\n함께 동승자를 구하고 이동해요!'} style={[styles.title, {position: 'absolute', zIndex: 999, top: 132}]}/>
      <Svg height="542" width="542">
        <Defs>
          <RadialGradient
            id="grad"

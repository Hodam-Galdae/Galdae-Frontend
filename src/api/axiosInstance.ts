...
axiosInstance.interceptors.request.use(
  async requestConfig => {
    ...
    console.log('🚀 [Axios Request] URL:', (requestConfig?.baseURL ?? '') + requestConfig?.url);
    console.log('🚀 [Axios Request] Method:', requestConfig.method?.toUpperCase());
    console.log('🚀 [Axios Request] Headers:', requestConfig.headers);
    console.log('🚀 [Axios Request] Data:', requestConfig.data);

    //제외 대상 URL이면 스킵
    if (EXCLUDED_URLS.includes(requestConfig.url || '')) {
      console.log('🚀 [Axios Request] 제외된 URL - 토큰 추가 스킵');
      return requestConfig;
    }
    if (MULTIPART_URLS.includes(requestConfig.url || '')) {
    ...
    console.log('🚀 [Axios Request] 최종 config:', requestConfig);
    return requestConfig;
  },
  error => {
